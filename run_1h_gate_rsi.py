"""Vectorized backtest of the RSI(14)-enhanced model on XAU/USD 1h data.

Same shape as run_1h_gate.py / src/backtest.py:run_backtest_vectorized, but:
  * edge table is computed via compute_edge_with_features() (adds rsi_at_open)
  * per-bar RSI(14) is computed on close
  * prob_up is the Sharpe-logistic base nudged by tanh((50-rsi)/20)*boost
  * side is decided from the RSI-aware prob_up vs prob_dn
...
from src.pipeline import (
    ...
    combine_probability_rsi,
)

THRESHOLDS = [0.50, 0.55, 0.60, 0.65, 0.70, 0.75]
GATE_HIT_RATE = 0.54
GATE_MIN_TRADES = 100
RSI_BOOST = 0.05  # probability nudge magnitude at extreme RSI


def run_backtest_rsi_vectorized(threshold: float, lam: float = 0.3,
                                 warmup_bars: int = 1000,
                                 rsi_boost: float = RSI_BOOST):
    """Vectorized walk-forward with per-bar RSI nudge.

    NOTE on lookahead: edge is computed once from the FULL history (same as
    the existing vectorized backtest), and RSI is computed once on close
    using all prior closes via Wilder's EMA (no future leak either).  This
    matches the bias level of the existing pipeline.
    """
    df = load_history("xau_usd_1h_real.csv")
    print(f"[rsi-backtest] Loaded {len(df)} bars "
            f"({df.index.min()} -> {df.index.max()})")

    # Compute edge (with rsi_at_open column, used as a slot-level context only)
    edge = compute_edge_with_features(df, period=14, min_count=30)
    has_rsi = edge["rsi_at_open"].notna().any()
    print(f"[rsi-backtest] Edge: {len(edge)} slots; rsi_at_open "
            f"present={has_rsi}")

    # Per-bar RSI on close (Wilder). NaN for first 14 bars — handled below.
    ...
    # Base prob_up from Sharpe-logistic (no sentiment here)
    df["prob_up_base"] = df.apply(
        lambda r: combine_probability_rsi(
            r, rsi=float(r["rsi"]) if pd.notna(r["rsi"]) else 50.0,
            lam=lam, sentiment=0.0, rsi_boost=rsi_boost,
        ),
        axis=1,
    )
    ...
    # Trade filter: past warmup, has edge, has RSI, prob >= threshold
    mask = (
...
def main() -> int:
    print("=" * 78)
    print("CEREBRUM 1H BACKTEST — RSI(14)-ENHANCED MODEL")
    print(f"RSI boost magnitude: +/- {RSI_BOOST} at extreme RSI; "
            "tanh((50 - rsi)/20) shaping")
    print("=" * 78)

    rows = []
    gate_pass = None
    for thresh in THRESHOLDS:
        trades_df, summary = run_backtest_rsi_vectorized(threshold=thresh)
        s = summary
        if s["total_trades"] == 0:
            rows.append({
    ...
    print()
    print("=" * 78)
    print("XAU/USD 1H BACKTEST — RSI(14)-ENHANCED — VECTORIZED")
    print("=" * 78)
    print(res.to_string(
        index=False,
