"""Vectorized backtest of the US10Y cross-asset model on XAU/USD 1h data.

Adds the 10-Year Treasury yield (US10Y, ^TNX) as a CAUSAL cross-asset feature
to the Sharpe-logistic base. US10Y moves positively with the dollar (and thus
NEGATIVELY to gold): higher real yields = stronger dollar, weaker gold. This
should provide edge that simple price-derived features (RSI/BB/momentum) cannot.

Features (per bar, leak-safe at bar open):
  * us10y_return_1h  — US10Y 1h percentage return, scalar = 10
  * us10y_return_4h  — US10Y 4h percentage return, scalar = 10
  * us10y_return_24h — US10Y 24h percentage return, scalar = 10
  Each contributes +/- us10y_boost/3 ≈ +/-0.033, bounded by +/- us10y_boost=0.10.
...
from src.pipeline import (
    load_history,
    load_us10y_history,
    merge_us10y_features,
    compute_edge_with_features,
    combine_probability_us10y,
)

THRESHOLDS = [0.50, 0.55, 0.60, 0.65, 0.70, 0.75]
GATE_HIT_RATE = 0.54
GATE_MIN_TRADES = 100
US10Y_BOOST = 0.10        # total max nudge = +/- 0.10
US10Y_SCALE = 10.0
WARMUP_BARS = 1000


def run_backtest_us10y_vectorized(threshold: float, lam: float = 0.3,
                                  warmup_bars: int = WARMUP_BARS,
                                  us10y_boost: float = US10Y_BOOST,
                                  us10y_scale: float = US10Y_SCALE):
    """Vectorized walk-forward with US10Y cross-asset nudger.

    The base probability comes from compute_edge_with_features() (Sharpe-logistic
    on per-(hour, day_of_week) slot). We then add a US10Y-return nudge that is
    signed negatively (yield up -> gold prob down).
    """
    df = load_history("xau_usd_1h_real.csv")
    print(f"[us10y-backtest] Gold: {len(df)} bars "
    ...
    # Merge US10Y (10Y Treasury yield) returns — leak-safe via asof backward
    us10y = load_us10y_history()
    df = merge_us10y_features(df, us10y)
    us10y_nonnull = int(df["us10y_return_1h"].notna().sum())
    print(f"[us10y-backtest] US10Y merged: {us10y_nonnull} bars with us10y_return_1h")
    ...
    # Apply US10Y nudger row-wise (leak-safe: US10Y return at bar t is observable
    # at the OPEN of bar t; we trade on gold return from t -> t+1)
    def _apply(row):
        return combine_probability_us10y(
            row,
            us10y_return_1h=row.get("us10y_return_1h", 0.0),
            us10y_return_4h=row.get("us10y_return_4h", 0.0),
            us10y_return_24h=row.get("us10y_return_24h", 0.0),
            lam=lam, sentiment=0.0,
            us10y_boost=us10y_boost, us10y_scale=us10y_scale,
        )
    ...
    mask = (
        ...
        & df["prob"].notna()
        & df["ret"].notna()
        & df["us10y_return_1h"].notna()    # require US10Y feature
        & df["us10y_return_4h"].notna()
        & df["us10y_return_24h"].notna()
        & (df["prob"] >= threshold)
    )
    ...
    trades_df = trade_bars[
        ["prob", "side", "mean_ret", "std_ret", "count",
         "us10y_return_1h", "us10y_return_4h", "us10y_return_24h",
...
def main() -> int:
    print("=" * 78)
    print("CEREBRUM 1H BACKTEST — US10Y CROSS-ASSET MODEL (XAU/USD vs ^TNX)")
    print(f"Nudge budget: +/- {US10Y_BOOST} (3 horizons × +/- {US10Y_BOOST/3:.4f})")
    print("=" * 78)
    ...
    for thresh in THRESHOLDS:
        trades_df, summary = run_backtest_us10y_vectorized(threshold=thresh)
        s = summary
        if s["total_trades"] == 0:
            rows.append({
    ...
    print()
    print("=" * 78)
    print("XAU/USD 1H BACKTEST — US10Y CROSS-ASSET FEATURE — VECTORIZED")
    print("=" * 78)
    print(res.to_string(
        index=False,
